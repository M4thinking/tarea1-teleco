...
def abrir_json(nombreArchivo):
  ...
  data = json.dumps(f)  # Guardamos el .json en un diccionario
...
def crear_cliente(Dict, Rut, nombre):
  ...
  if not Rut in Dict:
    ...
    Var["nombre"] = nombre # Creamos la llave con los datops del cliente
...
def crear_atencion(Dict, ID, descripcion):
  ...
  if not ID in Dict:
    Dict["atenciones"][ID] = Att #actualizamos el diccionario con la nueva atencion
